...
<section class="px-6 py-8">
    <nav class="md:flex md:justify-between md:items-center">
        ...
        <div class="mt-8 md:mt-0 flex items-center">
            @auth
                <span class="tex-xs font-bold uppercase">Welcome, {{ auth()->user()->name }}!</span>

                @can('admin')
                    <a class="ml-6 text-blue-500" href="/admin/posts/create">Administrate</a>
                @endcan
                ...
                <form method="POST" action="/newsletter" class="text-sm">
                    ...
                    <div class="lg:flex">
                        <div class="lg:py-3 lg:px-5 flex items-center">
                            <label for="email2" class="hidden lg:inline-block">
                                <img src="/images/mailbox-icon.svg" alt="mailbox letter">
                            </label>

                            <input id="email2" type="text" name="email" placeholder="Your email address"
    ...
    @if(session()->has('success'))
        <div class="fixed bg-blue-500 text-white py-2 px-4 rounded-xl bottom-3 right-3 text-sm" data-flash>
            <p>{{ session('success') }}</p>
        </div>
    @endif
</section>
